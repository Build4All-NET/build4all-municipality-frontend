...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  Future<void> _onSend() async {
    ...
    try {
      await AuthApiService().forgetPassword(email: _emailCtrl.text.trim());

      if (!mounted) return;
      setState(() => _isLoading = false);

      // ✅ STEP 1 done → go to OTP verify screen
      AppRouter.gotoVerifyResetCodeScreen(context, _emailCtrl.text.trim());
    } catch (e) {
      if (!mounted) return;
      setState(() => _isLoading = false);
              ...
              children: [
                const Center(
                  child: CircleAvatar(
                    ...
                    child: Icon(Icons.lock_reset, size: 30, color: Color(0xFF0D1B2A)),
                  ),
                ),
